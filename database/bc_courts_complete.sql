...
-- UPDATED SCHEMA: Cleaner court types with flags
-- ============================================================

-- Drop ALL existing tables/views
...
-- ============================================================
-- COURT TYPES TABLE (simplified to 3 types)
-- ============================================================
...
INSERT INTO court_types (name, description) VALUES
    ('Provincial', 'Provincial Court - handles criminal, family, small claims, traffic'),
    ('Supreme', 'Supreme Court - handles civil, criminal appeals, family law (divorce)'),
    ('Circuit', 'Circuit Court - staffed on hearing days only, documents filed at contact hub');

-- ============================================================
-- COURTS TABLE (with flags for Provincial/Supreme availability)
-- ============================================================
CREATE TABLE courts (
    id SERIAL PRIMARY KEY,
    ...
    has_supreme_filing BOOLEAN DEFAULT FALSE,  -- For Provincial courts that can file Supreme docs
    is_circuit BOOLEAN DEFAULT FALSE,
    
    -- Staffing
    is_staffed BOOLEAN DEFAULT FALSE,
    contact_hub VARCHAR(100),  -- For circuit courts: which staffed court handles admin
    ...
    -- Supreme Court specific (from uploaded PDF)
    supreme_scheduling_phone VARCHAR(50),
    supreme_scheduling_fax VARCHAR(50),
    supreme_scheduling_toll_free VARCHAR(50),
...
-- Insert courts
INSERT INTO courts (name, region_id, has_provincial, has_supreme, has_supreme_filing, is_circuit, is_staffed, contact_hub, address, phone, fax, civil_fax, criminal_fax, sheriff_phone, supreme_scheduling_phone, supreme_scheduling_fax, supreme_scheduling_toll_free, supreme_fax_filing, access_code, access_code_notes, notes)
...
SELECT 
    ...
    c.criminal_fax,
    c.sheriff_phone,
    c.supreme_scheduling_phone,
    c.supreme_scheduling_fax,
    c.supreme_scheduling_toll_free,
    c.supreme_fax_filing,
    c.access_code,
    c.access_code_notes,
    c.notes,
    -- Contacts
...
SELECT * FROM courts_full WHERE is_circuit = TRUE;

-- Provincial courts
CREATE OR REPLACE VIEW provincial_courts AS
SELECT * FROM courts_full WHERE has_provincial = TRUE;

-- Supreme courts
CREATE OR REPLACE VIEW supreme_courts AS
SELECT * FROM courts_full WHERE has_supreme = TRUE;
...
CREATE INDEX IF NOT EXISTS idx_courts_provincial ON courts(has_provincial);
CREATE INDEX IF NOT EXISTS idx_courts_supreme ON courts(has_supreme);
CREATE INDEX IF NOT EXISTS idx_courts_circuit ON courts(is_circuit);
...
UNION ALL SELECT 'Staffed Courts', COUNT(*) FROM courts WHERE is_staffed = TRUE
UNION ALL SELECT 'Circuit Courts', COUNT(*) FROM courts WHERE is_circuit = TRUE
...
UNION ALL SELECT 'Has Supreme Filing', COUNT(*) FROM courts WHERE has_supreme_filing = TRUE;
