// app/api/admin/update-circuit-addresses/route.ts
import { createClient } from '@supabase/supabase-js';
import { NextResponse } from 'next/server';

// Circuit court addresses gathered from BC Government and Provincial Court websites
const circuitCourtAddresses: Record<string, string | null> = {
...
export async function POST(request: Request) {
  try {
    ...
    if (secret !== process.env.ADMIN_SECRET && secret !== 'update-addresses-2024') {
    ...
    for (const court of courts || []) {
      ...
      if (address === null || address === undefined) {
        results.notFound.push(court.name);
        continue;
      }

      if (court.address === address) {
        results.skipped.push(court.name);
        continue;
      }
      ...
      if (updateError) {
        results.errors.push(`${court.name}: ${updateError.message}`);
      } else {
        results.updated.push(`${court.name}: ${address}`);
      }
    }
...
export async function GET() {
  return NextResponse.json({ 
    message: 'POST to this endpoint with ?secret=update-addresses-2024 to update circuit court addresses',
    addressCount: Object.keys(circuitCourtAddresses).length,
  });
}
